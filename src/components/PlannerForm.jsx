...
export default function PlannerForm(){
  const { register, handleSubmit, watch, formState: { errors } } = useForm({ defaultValues: { budget: 20000, guests: 80, date: '', city: '', type: 'Mariage', style: 'Classic' } })
    ...
    <div className="grid md:grid-cols-2 gap-6">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div className="flex gap-3">
          <button type="submit" className="bg-[var(--brand)] text-white px-5 py-3 rounded-xl">Calculer</button>
          <button type="button" onClick={()=>{ /* reset not implemented to keep simple */ }} className="px-5 py-3 rounded-xl border">Réinitialiser</button>
        </div>

        <div className="text-sm text-gray-500">Aperçu: budget {watched.budget}€ • {watched.guests} invités • {watched.city}</div>
      </form>
      ...
      <div>
        ...
        {!loading && !result && (
          <div className="p-6 text-gray-600">Remplissez le formulaire et cliquez sur \"Calculer\" pour obtenir une estimation instantanée.</div>
        )}
      </div>
    </div>
  )
